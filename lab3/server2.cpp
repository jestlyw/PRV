// Задача 2: асинхронный сервер, считает сумму чисел 
...
private:
    ...
    void handle_request(const std::string& line) {
        ...
        boost::asio::post(socket_.get_executor(), [this, self, line]() {
            ...
            std::string response = "Сумма: " + std::to_string(sum) + "\n";
            std::cout << "Отправляю: " << response;
            do_write(response);
        });
    }
...
int main() {
    try {
        ...
        Server s(io, 12346);
        std::cout << "Сервер (сумма) запущен на порту 12346\n";
        io.run();
